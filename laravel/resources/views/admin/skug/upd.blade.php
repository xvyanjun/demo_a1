    ...
    <div class="row data-type">
        ...
        <div class="col-md-10 data">
            <select name="" id="goods_id" class="form-control">
                ...
                @foreach($goods as $K=>$v)
                    <option value="{{$v->goods_id}}" {{$prop->goods_id==$v->goods_id ? "selected" : ""}}>{{$v->goods_name}}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-10 data">
            ...
            @foreach($v['a'] as $Kk=>$vv)
                <option value="{{$vv->val_id}}" class="val_id" {{$prop->val_id==$v->val_id ? "selected" : ""}} >{{$vv->val_name}}</option>
            @endforeach
            </select>
        </div>
        @endforeach
        ...
        <div class="col-md-2 title">商品库存</div>
		    <div class="col-md-10 data">
		        <input type="text" class="form-control" id="goods_stroe"  placeholder="库存" value="{{$prop->goods_stroe}}">
        </div>
        <div class="col-md-2 title">价格</div>
		    <div class="col-md-10 data">
		        <input type="text" class="form-control" id="price"  placeholder="价格" value="{{$prop->price}}">
		</div>
    </div>
...
<!--tab内容/-->
 ...
 <div class="btn-toolbar list-toolbar">
        <button class="btn btn-primary" ng-click="save()" data-id="{{$prop->id}}" id='tj'><i class="fa fa-save"></i>修改属性</button>
</div>
</body>
</html>
<script>
